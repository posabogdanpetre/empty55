...
module.exports = async ({ category = '' } = {}) => {
    ...
    if (results.length === 0) {
        summary = filterCategory
            ? `No banking products found in the "${filterCategory}" category.`
            : 'No banking products are currently available.'
    } else {
        summary = filterCategory
            ? `Found ${results.length} ${filterCategory} product${results.length === 1 ? '' : 's'}.`
            : `Found ${results.length} banking product${results.length === 1 ? '' : 's'}.`
    }
 ...
 *   GET ${process.env.API_BASE_URL}/banking-products?category=${category}
 *
 ...
 *     `${process.env.API_BASE_URL}/banking-products?${params.toString()}`,
